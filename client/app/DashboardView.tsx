        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
            <div style={styles.card}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    ...
                    <button style={{ ...styles.btn(false), border: '1px solid #D4AF37', background: 'transparent', color: '#D4AF37' }} onClick={() => window.open('/projector', '_blank')}>Projector 📺</button>
                </div>
            </div>
            ...
            <div style={styles.djCard(state.isDjMode)}>
                ...
                <button style={{ ...styles.btn(state.isDjMode), width: '100%', marginTop: '25px', height: '60px' }} onClick={handlers.toggleDJ}>{state.isDjMode ? 'DISABLE DJ ENGINE' : 'ENABLE DIGITAL DJ'}</button>
            </div>
        </section>
    );
};

// --- SUB-COMPONENT: INTELLIGENCE & CONFIG (COLUMN 3) ---
const IntelligencePanel = ({ state, handlers }: DashboardProps) => {
    ...
    return (
        <section style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
            <div style={styles.card}>
                <span style={styles.label}>TRACK INTELLIGENCE REPORT</span>
                ...
                <div style={styles.registryBox} className={isScanning ? 'pulse' : ''}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(212, 175, 55, 0.1)', paddingTop: '10px' }}>
                        <div>
                ...
                {dj.isrc && dj.isrc !== '--' && (
                    <button 
                        onClick={() => window.open(`https://www.discogs.com/search/?q=${dj.isrc}&type=release`, '_blank')} 
                        style={{ ...styles.outlineBtn, width: '100%', marginTop: '10px', height: '40px', fontSize: '0.65rem' }}
                    >
            ...
            {/* RESTORED: STATION CONFIGURATION & PROJECTOR CONTROLS */}
